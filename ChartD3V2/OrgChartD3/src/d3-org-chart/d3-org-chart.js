...
export class OrgChart {
    ...
    downloadImage({ node, scale = 2, isSvg = false, save = true, onAlreadySerialized = d => { }, onLoad = d => { } }) {
        ...
        const attrs = this.getChartState();
        if (isSvg) {
            let source = serializeString(svgNode, attrs);
            //add xml declaration
        ...
        var url = 'data:image/svg+xml; charset=utf8, ' + encodeURIComponent(serializeString(svgNode, attrs));
        ...
        function serializeString(svg, attrs) {
            const xmlns = 'http://www.w3.org/2000/xmlns/';
            const xlinkns = 'http://www.w3.org/1999/xlink';
            const svgns = 'http://www.w3.org/2000/svg';
            svg = svg.cloneNode(true);

            svg.querySelectorAll('.nodeBig_ae985208').forEach(el => {
                if (el.hasAttribute("data-nodeId")) {
                    let nodeId = el.getAttribute("data-nodeid");
                    if (nodeId) {
                        
                        const newItem = getCustomNodePrint(nodeId, attrs);
        ...
        // Custom print node 
        function getCustomNodePrint(nodeId, attrs) {
            if (!attrs.data || attrs.data.length == 0) {
                console.log('ORG CHART - Data is empty')
                return '';
            }
            else {
                const currentNode = attrs.data.find(function (obj) { return obj.id == nodeId })
                if (currentNode) {
                    // Get Image Base64                    
                    var div = document.createElement('div');
                    div.innerHTML = PrintHtmlNode(currentNode);
                    return div;
                }
            }
        }

        function PrintHtmlNode(currentNode) {
            var htmlElm =  
            `<div style="font-family: 'Segoe UI', 'Segoe UI Web (West European)', 'Segoe UI', -apple-system, BlinkMacSystemFont, Roboto, 'Helvetica Neue', sans-serif;-webkit-font-smoothing: antialiased;font-size: 14px;font-weight: 400;box-shadow: none;margin: 0px;padding: 0px;box-sizing: border-box;color: rgb(50, 49, 48);position: relative;height: 72px;min-width: 72px;display: flex;align-items: center;">
                <div style="position: relative;text-align: center;flex: 0 0 auto; height: 72px;width: 72px;">
                    <div style="border-radius: 50%;color: rgb(255, 255, 255);font-size: 28px; font-weight: 600;line-height: 72px;height: 72px;background-color: rgb(79, 107, 237);">
                        <span>HK</span>                        
                    </div>
                </div>
                <div style="padding: 0px 24px 0px 16px;min-width: 0px;width: 100%;text-align: left; display: flex;flex-direction: column;justify-content: space-around;">
                    <div style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: rgb(50, 49, 48);font-weight: 400;font-size: 20px;">${currentNode.name}</div>
                    <div style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: rgb(96, 94, 92);font-weight: 400;font-size: 14px;">${currentNode.positionName}</div>
                    <div style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: rgb(96, 94, 92);font-weight: 400;font-size: 14px;">${currentNode.office}</div>
                </div>
            </div>`;
